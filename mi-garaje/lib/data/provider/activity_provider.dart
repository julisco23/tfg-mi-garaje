...
class ActivityProvider extends ChangeNotifier {
  ...
  Future<void> loadActivities(String vehicleId, String ownerId, String ownerType) async {
    _activities = await carService.getActivities(vehicleId, ownerId, ownerType);
    notifyListeners();
  }

  // Métodos para manejar actividades
  Future<void> addActivity(String vehicleId, String ownerId, String ownerType, Activity activity) async{
    await carService.addActivity(vehicleId, activity, ownerId, ownerType);
    _activities.add(activity);
    notifyListeners();
  }

  Future<void> deleteActivity(String vehicleId, String ownerId, String ownerType, Activity activity) async{
    await carService.deleteActivity(vehicleId, activity.idActivity!, ownerId, ownerType);
  ...
  Future<void> deleteAllActivities(String ownerId, String ownerType, String typeName, {String type = "custom"}) async {
  ...
  Future<void> editAllActivities(String ownerId, String ownerType, String oldName, String newName, {String type = "custom"}) async {
    await carService.editAllActivities(ownerId, oldName, newName, type, ownerType);
  }
}
